                ...
                async for response in self.session.receive():
                    ...
                    if response.tool_call:
                        ...
                        try:
                            ...
                            print(f"[JARVIS] ⚠️ Tool response failed: {te}")

            except Exception as e:
                # Catch 1008 or connection closed and allow the run() loop to reconnect
                if "1008" in str(e) or "closed" in str(e).lower():
                    print(f"[JARVIS] 🔌 Session closed (1008/Disconnect): {e}")
                else:
                    print(f"[JARVIS] ❌ Recv Error: {e}")
                    traceback.print_exc()
                
                # Signal the TaskGroup that we need to restart the session
                break 
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    # Wait for Boot sequence + Vision system
                    print("[JARVIS] ⏳ Waiting for Boot Sequence...")
                    while not self.ui.boot_finished:
                        await asyncio.sleep(0.5)
                    
                    # Hindi startup greeting
                    await asyncio.sleep(0.5)
                    await session.send_client_content(
                        turns={"parts": [{"text": (
                            "Greet the user now in Hindi with a short, confident JARVIS-style greeting. "
                            "Something like: 'Namaste, sir. Sabhi systems online hain. Main aapki seva mein hazir hoon.' "
                            "Keep it under 2 sentences. Be natural, not robotic."
                        )}]},
                        turn_complete=True,
                    )
...
def main():
    ...
    try:
        from core.vision_manager import start_vision
        _vm = start_vision(ui=ui)
    except Exception as _ve:
        print(f"[JARVIS] Vision manager not started: {_ve}")
